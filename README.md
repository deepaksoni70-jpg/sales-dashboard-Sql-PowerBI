...
## ⚠️ About the .pbix file
This repo does **not** include a `.pbix` — Power BI Desktop (Windows-only) is required to create one, and it
can't be generated by code. **See [`POWERBI_GUIDE.md`](POWERBI_GUIDE.md)** for a 15-20 min step-by-step guide
(with copy-paste DAX measures) to turn the CSVs + SQL in this repo into a real `.pbix`, plus guidance on what
link to actually put on your resume (a raw `.pbix` file usually isn't openable by recruiters — a published
Power BI Service link or an exported PDF works better).
